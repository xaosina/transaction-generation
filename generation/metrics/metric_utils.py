...
class MetricEstimator:
    ...
    def __init__(self, gt_save_path, gen_save_path, config, logger, train_state):
        ...
        self.metrics = [metric(metric.__name__) for metric in METRICS]

    def set_name_fixes(self, name_postfix="", name_prefix=""):
        self.name_postfix = name_postfix
        self.name_prefix = name_prefix

    def estimate_all(self, orig_path, generated_path, shuffle_clients=False, seed=0):
        ...
        for parti, path in zip(["gt", "gen"], [orig_path, generated_path]):
            df = pd.read_csv(path)
            df = (
                df[[subject_key, target_key]]
                .groupby(subject_key)
                .tail(self.config["EST_GEN_LEN"] + self.config["EST_HIST_LEN"])
                .reset_index(drop=True)
            )
            assert (df.groupby(subject_key).size() >= self.config["EST_GEN_LEN"] + self.config["EST_HIST_LEN"]).all()
            dfs[parti]["hist"] = df.groupby(subject_key).head(self.config["EST_HIST_LEN"]).reset_index(drop=True)
            dfs[parti]["preds"] = df.groupby(subject_key).tail(self.config["EST_GEN_LEN"]).reset_index(drop=True)

        if set(dfs["gt"]["pred"].client_id) != set(dfs["gen"]["pred"].client_id):
        ...
        for metric in self.metrics:
            results[metric.name] = metric(dfs, subject_key, target_key)
        ...
