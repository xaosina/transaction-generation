...
class EDMLoss:
    ...
    def __call__(
        ...
        y = data
        n = torch.randn_like(y) * append_dims(sigma, y.ndim)
        assert n.ndim == y.ndim
        ...
        if match_emb_size is not None:
            ...
            return append_dims(weight, mse.ndim) * mse
        loss = append_dims(weight, target.ndim) * ((D_yn - target) ** 2)
