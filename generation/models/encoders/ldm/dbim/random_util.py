...
def get_generator(generator, num_samples=0, seed=0, device='cpu'):
    if generator == "dummy":
        return DummyGenerator()
    elif generator == "determ":
        return DeterministicGenerator(num_samples, seed, device)
    elif generator == "determ-indiv":
        return DeterministicIndividualGenerator(num_samples, seed, device)
    else:
...
class DeterministicGenerator:
    ...
    def __init__(self, num_samples, seed=0, device='cpu'):
...
class DeterministicIndividualGenerator:
    ...
    def __init__(self, num_samples, seed=0, device='cpu'):
        ...
        if torch.cuda.is_available():
            self.rng_cuda = [torch.Generator(device) for _ in range(num_samples)]
        self.set_seed(seed)
...
class BatchedSeedGenerator:
    ...
    def __init__(self, seeds=None, device='cpu'):
        self.num_samples = len(seeds)
        if torch.cuda.is_available():
            self.rng = [torch.Generator(device) for _ in range(self.num_samples)]
        else:
