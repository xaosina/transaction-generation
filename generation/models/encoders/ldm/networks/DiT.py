...
class DiT(LDMDenoiser):
    ...
    def __init__(
        ...
        self.num_rows = generation_len + reference_len
        ...
        self.history_embedding_embedder = nn.Identity()
        if self.is_history_embedding_conditional:
            self.history_embedding_embedder = nn.Linear(
                self.history_embedding_dim, 
                self.hidden_size
            )
        ...
        self.plug_reference_seq = nn.Parameter(
            torch.zeros(1, self.reference_len, self.latent_size), 
            requires_grad=True,
        )
        self.plug_history_embedding = nn.Parameter(
            torch.zeros(1, self.history_embedding_dim),
            requires_grad=True,
        )

        # history time + history_embedding plug
        self.plug_c_reference = nn.Parameter(
            torch.zeros(1, self.reference_len, hidden_size),
            requires_grad=True,
        )
    ...
    def forward(
        ...
        **kwargs
        ...
        assert reference.size(1) == self.reference_len
        ...
        assert x.size(0) == reference.size(0) # bs
        assert x.size(2) == reference.size(2) # latent dim
        ...
        if self.is_history_embedding_conditional:
            if history_embedding is None:
                history_embedding = self.plug_history_embedding.repeat(bs, 1)
            assert history_embedding.size(1) == self.history_embedding_dim
            history_embedding_emb = self.history_embedding_embedder(history_embedding)
            assert history_embedding_emb.size(0) == c.size(0)
            assert history_embedding_emb.size(1) == c.size(2)
            c = c + history_embedding_emb.unsqueeze(1) # (bs, L_gen, D)
        ...
        assert x.shape == (bs, self.reference_len + self.generation_len, self.latent_size)
        return x[:,self.reference_len:, :]
