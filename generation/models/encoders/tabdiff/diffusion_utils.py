...
#         self.tokenizer = Tokenizer(d_numerical, categories, d_token, bias = bias)
#         self.encoder = Transformer(num_layers, 3*d_token, n_head, d_token, factor)
#         d_in = 10*d_token * (d_numerical + len(categories))
#         self.mlp = MLPDiffusion(d_in, dim_t=dim_t, use_mlp=use_mlp)
#         self.decoder = Transformer(num_layers, d_token, n_head, d_token, factor)
#         self.detokenizer = Reconstructor(d_numerical, categories, d_token)
#         self.model = nn.ModuleList([self.tokenizer, self.encoder, self.mlp, self.decoder, self.detokenizer])
...
#         ls,bs = x_num.shape[0],x_num.shape[1]
#         ## e: (B,L,F*d_token)
#         e = self.tokenizer(x_num, x_cat)
#         hst = self.tokenizer(hist_num,hist_cat)
#         e_input,h_input = e,hst
#         ## combined_input: (B,L1+L2,F*d_token)
#         combined_input = torch.cat([e_input,h_input],dim=1)
#         ## y: (B,L1+L2,F*d_token)
#         y = self.encoder(combined_input)
#         ## input of mlp: (bs,d_in) and (bs,)
#         ## output: (bs,d_in)
#         pred_y = self.mlp(y.reshape(y.shape[1], -1), timesteps)
#         pred_e = self.decoder(pred_y.reshape(*y.shape))
#         pred_e_curr = pred_e[:,:hist_len,:]
#         x_num_pred, x_cat_pred = self.detokenizer(pred_e_curr)
#         x_cat_pred = torch.cat(x_cat_pred, dim=-1) if len(x_cat_pred)>0 else torch.zeros_like(x_cat).to(x_num_pred.dtype)
...
#         L_token_out = d_numerical + categories.size(0)
...
#         timesteps_repeated = timesteps.repeat_interleave(L)
