...
class BaselineHP(BaseGenerator):
    ...
    def generate(self, hist: GenBatch, gen_len: int, with_hist=False) -> GenBatch:
        ...
        generated = PredBatch(
            ...
            cat_features={self._feature_name: torch.tensor(np.stack(gen_marks).T, dtype=torch.float32)},
        )
...
class BaselineHMM(BaseGenerator):
    ...
    def generate(self, hist: GenBatch, gen_len: int, with_hist=False) -> GenBatch:
        ...
        for _, seq in enumerate(zip(feature.T, lengths)):
            ...
            best_model = select_hmm_by_bic(obs=obs, M=M, max_K=10, n_iter=50, tol=1e-4, restarts=3)
        ...
        generated = PredBatch(
            ...
            num_features=None,
            num_features_names=None,
            cat_features={self._feature_name:
                          torch.tensor(np.stack(gen_marks).T, dtype=torch.float32)},
        )
        return generated.to_batch()
