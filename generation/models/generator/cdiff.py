...
class CrossDiffusionModel(BaseGenerator):
    ...
    def generate(
        ...
        pred_cat = torch.empty(hist_cat.size(0), gen_len, hist_cat.size(-1), 0).to("cuda")
        pred_num = torch.empty(hist_num.size(0), gen_len, hist_num.size(-1), 0).to("cuda")
        ...
        pred_num = pred_num.mean(dim=-1)
        assert pred_num.ndim == 3
    ...
    def toGenBatch(self, pred_cat, pred_num, features_name):
        ...
        return GenBatch(
            ...
            time=pred_num[:, :, 0],
            index=None,
            num_features=pred_num[:, :, 1:] if pred_num.size(-1) > 1 else None,
