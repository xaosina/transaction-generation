...
class CrossDiffusionModel(BaseGenerator):
    ...
    def forward(self,x : GenBatch) -> torch.Tensor:
        ...
        if hist_batch.num_features_names:
            ...
            hist_num = torch.cat([hist_batch.time.unsqueeze(-1),hist_batch.num_features],dim = -1)
        else:
        ...
        tgt_num,tgt_cat = tgt_num.permute(1,0,2),target_batch.cat_features.permute(1,0,2)
