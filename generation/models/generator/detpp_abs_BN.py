...
class DeTPP_abs_BN(BaseGenerator):
    def __init__(self, data_conf: LatentDataConfig, model_config: ModelConfig):
        ...
        self.norm_layer = nn.BatchNorm1d(self.autoencoder.encoder.output_dim)
    ...
    def forward(self, x: GenBatch) -> PredBatch:
        ...
        ## add regulerizer loss, such that x is in latent space.
        flat_x = ad_x.view(-1,self.autoencoder.encoder.output_dim)
        norm_x = self.norm_layer(flat_x).view(L,B * self.k_output,-1)
    ...
    def generate(
        ...
        with torch.no_grad():
            for _ in range(0, gen_len, self.k_gen):
                ...
                x_shape = x.tokens.shape
                flat_x = ad_x.view(-1,self.autoencoder.encoder.output_dim)
                norm_x = self.norm_layer(flat_x).view(x_shape)
