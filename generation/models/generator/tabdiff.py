...
class ContinuousDiscreteDiffusionGenerator(BaseGenerator):
    ...
    def forward(self, x: GenBatch) -> torch.Tensor:
        ...
        # ## get the predict results from GRU
        # pred_batch = self.cond_generator.sample(history_batch,32)
        # ## get the category:small_group from GRU and use it as condition
        # cond_seq = pred_batch.cat_features[:,:,1]
