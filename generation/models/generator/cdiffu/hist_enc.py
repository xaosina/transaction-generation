...
class HistoryEncoder(nn.Module):
    def __init__(self,
        ...
        # 为所有 F 个类别特征定义独立的嵌入层
        self.all_cat_emb_layers = nn.ModuleList([
            nn.Embedding(vocab_size, transformer_dim)
            for i, vocab_size in enumerate(all_dynamic_vocab_sizes)
        ])

        # 定义投影层，用于融合所有类别嵌入 (F*D -> D)
        cat_concat_dim = self.num_all_cat_features * transformer_dim
        self.dynamic_cat_proj = nn.Linear(cat_concat_dim, transformer_dim)
        
        dynamic_feature_dim_sum = transformer_dim # 类别特征贡献 D 维度
        ...
        dynamic_feature_dim_sum += transformer_dim # 时间编码贡献 D 维度

        # B. 额外的 M 个数值特征的投影层
        if num_extra_numerical_features > 0:
            # 投影层：将 M 维投影到 D 维
            self.dynamic_num_proj = nn.Linear(num_extra_numerical_features, transformer_dim)
            dynamic_feature_dim_sum += transformer_dim # 额外数值特征贡献 D 维度
    ...
    def forward(self, hist_x, hist_e):
        ...
        for i in range(self.num_all_cat_features):
            ...
            cat_emb = self.all_cat_emb_layers[i](cat_input) # [B, L', D]
            cat_embeddings.append(cat_emb)

        # C. 投影融合 [B, L', F*D] -> [B, L', D]
        cat_concat = torch.cat(cat_embeddings, dim=-1)
        dynamic_cat_fused = self.dynamic_cat_proj(cat_concat)
        dynamic_features_list.append(dynamic_cat_fused)
