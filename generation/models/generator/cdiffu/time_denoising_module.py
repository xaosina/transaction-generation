...
class TimeDenoisingModule(nn.Module):
    def __init__(self, transformer_dim=32, num_classes=10, n_steps=100, transformer_heads=2,
        ...
        x_dim = len_numerical_features-1
        self.x_dim = x_dim
    ...
    def forward(self, x, e, t, hist,cat_order):
        ...
        x_time_emb = self.temporal_enc(x[:,:,0])
        x_num_emb = self.x_input_layer(x[:,:,1:])
        e_emb = []
        ...
        combined_features = torch.cat([x_time_emb, x_num_emb, e_emb, t_emb_seq], dim=-1) + order_emb
    ...
    def temporal_enc(self, dt_seq):
        ...
        position_vec = torch.tensor(
            [math.pow(10000.0, 2.0 * (i // 2) / int(self.feature_dim)) for i in range(int(self.feature_dim))],
            device= dt_seq.device) 
        result = dt_seq.unsqueeze(-1) / position_vec
        result[:, :, 0::2] = torch.sin(result[:, :, 0::2])
        result[:, :, 1::2] = torch.cos(result[:, :, 1::2])
        return result
