...
import math
from .utils import (
    log_onehot_to_index_multi_task,
    softmax_to_logits_multi_task,
    index_to_log_onehot_multi_task,
)
...
def logsumexp_multi_task(log_probs, num_class):
    ...
    logsumexp_results = [
        torch.logsumexp(chunk, dim=dim_to_split, keepdim=True) for chunk in chunks
    ]
...
class DiffusionTypeModel(torch.nn.Module):
    ...
    def q_posterior(self, log_x_start, log_x_t, t):
        ...
        log_EV_xtmin_given_xt_given_xstart = unnormed_logprobs - logsumexp_multi_task(
            unnormed_logprobs, self.num_classes_list
        )
    ...
    def q_sample(self, log_x_start, t):
        ...
        #
        log_sample = log_sample_categorical_multi_task(
            log_EV_qxt_x0, self.num_classes_list
        )

        return log_sample

    def nll(self, log_x_start, hist_emb=None):
    ...
    def kl_prior(self, log_x_start):
        ...
        for i in self.num_classes_list:
            log_half_prob[:, idx : idx + i, :] = -torch.log(
                i * log_half_prob[:, idx : idx + i, :]
            )
            idx += i
        kl_prior = self.multinomial_kl_multi_task(
            log_qxT_prob, log_half_prob, self.num_classes_list
        )
        return kl_prior

    def compute_Lt(self, log_x_start, log_x_t, t, dt, hist, detach_mean=False, hist_emb=None):
        #
        log_true_prob = self.q_posterior(log_x_start=log_x_start, log_x_t=log_x_t, t=t)

        log_model_prob = self.p_pred(log_x=log_x_t, t=t, dt=dt, hist=hist, hist_emb=hist_emb)

        if detach_mean:
            log_model_prob = log_model_prob.detach()

        kl = self.multinomial_kl_multi_task(
            log_true_prob, log_model_prob, self.num_classes_list
        )
    ...
    def _train_loss(self, x, dt, hist, t, pt, hist_emb=None):
        ...
        if self.loss_type == "vb_stochastic":
            ...
            kl = self.compute_Lt(
                log_x_start, self.q_sample(log_x_start=log_x_start, t=t), t, dt, hist, hist_emb=hist_emb
            )
    ...
    def compute_loss(self, x, dt, hist, cat_order, t, pt, hist_emb=None):
        #
        # x: e, dt: x, hist: hist,
        ...
        num_elem = x.size(0)
        loss = self._train_loss(x, dt, hist, t, pt, hist_emb=hist_emb)
