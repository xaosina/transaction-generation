...
class LatentDiffusionGenerator(BaseGenerator):
    ...
    def __init__(self, data_conf: LatentDataConfig, model_config: ModelConfig):
        ...
        self.reference_len = self.encoder.reference_len
    ...
    def get_reference_seq(self, hist: GenBatch, latent_hist):
        hist = deepcopy(hist)
        if (self.reference_generator is None) and (self.reference_len > 0):
            latent_ref = get_seq_tail(latent_hist, self.reference_len)
            orig_ref = hist.tail(self.reference_len)
