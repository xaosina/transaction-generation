...
def preprocess_full_alphabattle(
    ...
    for p, cn, pth in zip(["train", "test"], clients_number, raw_data_path):
        ...
        df = feature_drop(
            df,
            "payment_system",
            drop_cats=df["payment_system"].value_counts().index[-1:].tolist(),
            user_col="app_id",
        )
        df = feature_drop(
            df,
            "mcc_category",
            drop_cats=df["mcc_category"].value_counts().index[-1:].tolist(),
            user_col="app_id",
        )
        df = feature_drop(
            df,
            "card_type",
            drop_cats=df["card_type"].value_counts().index[-20:].tolist(),
            user_col="app_id",
        )
