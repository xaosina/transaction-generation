...
def select_clients(df, feature_name, cn, seed=42):
    clients_with_counts = df.groupBy(feature_name).count()
    sampled_clients = clients_with_counts.orderBy(F.rand(seed)).limit(cn)
...
def seq_len_filter(df, user_id, min_seq_len=512):
    ...
    return df.join(seq_len, on=user_id, how="inner").filter(F.col("seq_len") > min_seq_len).drop('seq_len')
...
def main(data_path, dataset_name, clients_number=1_000):
