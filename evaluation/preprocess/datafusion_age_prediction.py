...
def parse_args():
    ...
    parser.add_argument("-t", "--tabsyn", type=bool, action="store_true")
...
def prepare_tabsyn(data_path, n_rows, metadata):
    ...
    print(df.columns)
    df[metadata["ordering_columns"][0]] = df.groupby("user_id")[
        "time_diff_days"
    ].cumsum()
...
def prepare_orig(data_path, n_rows, sample_size, metadata):
    ...
    if (sample_size > 0) and (sample_size < len(user_ids)):
        print(f"Reducing original data from {len(user_ids)} to {sample_size}.")
        user_ids = df["user_id"].unique()
        gen = np.random.default_rng(0)
        train_ids = pd.Series(
            gen.choice(user_ids, size=sample_size, replace=False),
            name="user_id",
        )
    ...
    if n_rows == 1:
        ...
        range_number = df.groupby("user_id").cumcount()
        df["user_id"] = range_number.astype(str) + "_" + df["user_id"].astype(str)
        ...
        time_offset = df.groupby("user_id")[metadata["ordering_columns"][0]].transform(
            "first"
    ...
    print(f"Prepare orig finished. {df['user_id'].nunique()} sequences total.")
    return df
